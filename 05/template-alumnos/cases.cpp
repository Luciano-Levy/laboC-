...
bool test03_indiceMinSubsec(){
    ...
    bool res = asegurar(indiceMinSubsecDes(unElemento, 0,0) == 0, true);
    res &= asegurar(indiceMinSubsecDes(minimoAlPrincipio, 0,4) == 0, true);
    res &= asegurar(indiceMinSubsecDes(minimoAlFinal, 0,5) == 5, true);
    res &= asegurar(indiceMinSubsecDes(negativos, 0,5) == 2, true);
    res &= asegurar(indiceMinSubsecDes(subsec, 1,4) == 2, true);
